...
void cetakUtas(int idUtas, ListKicau kList, Account account, AccountList *listAccount, Graf teman)
{
    ...
    {
        printf("ketemunggasih\n");
        if (kList.kicau[i].utasKicau != NULL)
        {
            printf("ketemunggasih %d %d\n",kList.kicau[i].utasKicau->IDUtas,idUtas);

            if (kList.kicau[i].utasKicau->IDUtas == idUtas)
            {
                printf("ketemunggasih %d\n",kList.kicau[i].utasKicau->IDUtas);
                isFound = true;
...
void hapusUtas(int idUtas, int index, ListKicau *kList, Account account)
{
    ...
    {
        if (!WordEqual(kList->kicau[i].author, account.username[0]))
        {
        ...
        else
        {
            ...
            {
                kSambungAdd kSam = kList->kicau[i].utasKicau->content;
                kSambungAdd delKSam;
                if (index == 1)
                {
                    delKSam = kSam;
                    kSam = kSam->next;
                    kList->kicau[i].utasKicau->content = kSam;
                    if (kSam==NULL){
                        kList->kicau[i].utasKicau = NULL;
                    }
                }
                else
                {
