...
AddressBalasan NewTreeNode(typeBalasan root)
...
TreeBalasan NewTreeBalasan(typeBalasan root, TreeBalasan child)
...
{
	AddressBalasan t = NewTreeNode(root);
	if (t != NULL)
	{
...
void CreateTreeBalasan(typeBalasan root, TreeBalasan child, TreeBalasan *t)
/* I.S. : Sembarang
...
boolean NoChildBalasan(TreeBalasan t)
...
boolean OneChildBalasan(TreeBalasan t)
...
void insertTreeBalasan(TreeBalasan t, int parentID, typeBalasan child) {
	boolean found = false;
	TreeBalasan c = NewTreeBalasan(child, NULL);
